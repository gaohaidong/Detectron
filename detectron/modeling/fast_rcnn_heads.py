...
def add_fast_rcnn_outputs(model, blob_in, dim):
    ...
    if not model.train:  # == if test
        ...
        model.Softmax('cls_score_', 'cls_prob', engine='CUDNN')
...
def add_fast_rcnn_losses(model):
    ...
    cls_prob, loss_cls = model.net.SoftmaxWithLoss(
        ['cls_score_', 'labels_int32'], ['cls_prob', 'loss_cls'],
        scale=model.GetLossScale()
    )
    loss_bbox = model.net.SmoothL1Loss(
        [
            'bbox_pred_', 'bbox_targets', 'bbox_inside_weights',
...
def add_roi_2mlp_head_roi_conv(model, blob_in, dim_in, spatial_scale):
    ...
    model.FC(current, 'fc6_', dim_in * roi_size * roi_size, fc_dim)
    model.Relu('fc6_', 'fc6_')
    model.FC('fc6_', 'fc7_', hidden_dim, hidden_dim)
    model.Relu('fc7_', 'fc7_')
    return 'fc7_', hidden_dim
