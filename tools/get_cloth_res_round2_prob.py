...
if __name__ == '__main__':
    ...
    round2_csv = '{}_round2_prob_merge.csv'.format(res_csv[:-4])
    defect_codes = {'zhadong':1, 'bianzhadong':1, 'maoban':2, 'cadong':3, 'maodong':4, 'zhixi':5, 'diaojing':6, 'quejing':7, 'bianquejing':7, 'tiaohua':8, 'jingtiaohua':8, 'youzi':9, 'wuzi':9, 'huangzi':9}
    ...
    with open(res_csv) as f:
        for line in f.readlines():
            ...
            if items[0][-4:] == '.jpg':
                ...
                max_thresh[0] = 1 - sum([max_thresh[i] for i in range(1,11)])
                if max_thresh[0] < 0:
                    max_thresh = softmax_notzero(max_thresh)
                for i in range(11):
